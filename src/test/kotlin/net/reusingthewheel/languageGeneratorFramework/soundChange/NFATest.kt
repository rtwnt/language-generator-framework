        ...
        describe("getMatchingPrefix method") {
            setup.forEach { (key, value) ->
                describe("called on ${key.second}") {
                    val automaton = key.first()
                    it("throws IllegalArgumentException when given an empty sequence of symbols") {
                        Assertions.assertThatThrownBy { automaton.getMatchingPrefix(listOf()) }.isInstanceOf(IllegalArgumentException::class.java)
                    }
                    ...
                    value.forEach { (symbols, expected) ->
                        it("returns MatchResult with isMatchDetected = " +
                            ...
                            Assertions.assertThat(automaton.getMatchingPrefix(symbols)).usingRecursiveComparison().isEqualTo(expected)
                        }
                    }
                }
            }
